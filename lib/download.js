...
const OUTPUT = "./data/golf-clubs.json";
const cacheDir = process.env.XDG_CACHE_HOME;
const cached = cacheDir ? join(cacheDir, "navtee", "golf-clubs.json") : null;
...
if (cached && await exists(cached)) {
  ...
  console.log("Restored golf-clubs.json from cache");
}

// Skip download if file already exists
if (await exists(OUTPUT)) {
  console.log(`${OUTPUT} already exists, skipping download`);
  process.exit(0);
}
...
(async () => {
  ...
  await fs.writeFile(OUTPUT, JSON.stringify(geojson), "utf8");
  console.log(`Wrote ${features.length} clubs to ${OUTPUT}`);

  // Save to cache
  if (cached) {
    await fs.mkdir(join(cacheDir, "navtee"), { recursive: true });
    await fs.copyFile(OUTPUT, cached);
    console.log("Saved golf-clubs.json to cache");
  }
})();
