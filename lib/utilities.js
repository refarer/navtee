...
export function addCourses(golfClubs) {
  ...
  // Group any unassigned holes into a single fallback course
  const unassignedHoles = holes.filter((hole) => !hole.properties.courseId);
  if (unassignedHoles.length > 0) {
    const fallbackCourseId = unassignedHoles[0].properties.id;
    unassignedHoles.forEach((hole) => {
      hole.properties.courseId = fallbackCourseId;
    });
  }
